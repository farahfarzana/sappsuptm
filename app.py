...
def generate_graph_page():
    ...
    if uploaded_file:
        ...
        if groupby_column == 'All':
            ...
            # Add Status Risk graph separately
            df_grouped_status_risk = df.groupby(by=['Status Risk'], as_index=False)[output_columns].count()
            fig_status_risk = go.Figure()
            for status in df_grouped_status_risk['Status Risk'].unique():
                df_status_risk = df_grouped_status_risk[df_grouped_status_risk['Status Risk'] == status]
                trace_status_risk = go.Bar(
                    x=[f"All {groupby_column}"],
                    y=df_status_risk['Total Students'],
                    text=df_status_risk['Total Students'],
                    textposition='auto',
                    marker_color=colors[status],
                    name=status,
                )
                fig_status_risk.add_trace(trace_status_risk)

            fig_status_risk.update_layout(
                title=f'<b>Total Students by Status Risk - Grouped Bar Chart</b>',
                xaxis_title=groupby_column,
                yaxis_title='Total Students',
                barmode='group',
            )

            # Add Status Risk graph to the main plot
            fig.add_trace(fig_status_risk.data[0])

            fig.update_layout(
                title_text="Total Students by Different Categories - Stacked Column Chart",
                showlegend=False,
